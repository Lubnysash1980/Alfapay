...
const TOKEN_AMOUNT = 4900000000000000;              // скільки токенів домінтити
...
(async () => {
  try {
    ...
    // Спроба автоматично створити ATA для Token-2022
    ...
    try {
      receiverTokenAccount = await getOrCreateAssociatedTokenAccount(
        ...
        "confirmed",
        undefined,
        TOKEN_2022_PROGRAM_ID // важливо для Token-2022
      );
      console.log("✅ ATA знайдено/створено:", receiverTokenAccount.address.toBase58());
    } catch (err) {
      console.warn("⚠️ Не вдалося автоматично створити ATA. Пробуємо вручну...");
      ...
      const tx = new Transaction().add(
        createAssociatedTokenAccountInstruction(
          ...
          mintPublicKey,         // mint
          TOKEN_2022_PROGRAM_ID  // programId для Token-2022
        )
      );
    ...
    const txMint = await mintTo(
      ...
      TOKEN_AMOUNT,
      [], // multisigSigners
      { programId: TOKEN_2022_PROGRAM_ID } // важливо для Token-2022
    );
